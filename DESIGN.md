...
Headroom: ~463 KB flash, ~191 KB RAM. Pet sprite data (compiled-in headers) will be the next meaningful flash budget increase — a multi-frame animation at 60×60px 1-bit runs ~450 bytes per frame. 20 frames ≈ 9 KB. Nowhere near the limit. Sprite sheet tooling is already implemented in `tools/convert_image.py` (`--sprite-w`, `--sprite-h`, `--names` flags).
